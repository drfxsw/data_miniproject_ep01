...
hypothesis_data = {
    ...
    'tack': [
                ...
                {
                    'title': '가설3: 관련 질병의 유무는 폐암 환자의 생존률에 영향을 준다.',
                    'hypothesis': {
                                'h0': '관련 질병의 유무는 폐암 환자의 생존률에 영향이 없다.',
                                'h1': '관련 질병의 유무는 폐암 환자의 생존률에 영향이 있다.'
                            },
                    'folder_path': 'images/tack_page/hypothesis_03',
                    'images': load_images_from_static('images/tack_page/hypothesis_03', 'chart'),
                    'results': {
                        '학습 데이터 예측 정확도' : '0.7805692307692308',
                        '검증 데이터 예측 정확도' :  '0.7802295384615384'
                        

                    },
                    'conclusion': '원시데이터에서 1(생존)의 개수가 약 7만 개, 0(사망)의 개수가 약 25만 개로 불균형이 존재하여 모델이 학습 과정에서 사망(0)에 편향되었다. 그 결과, 사망한 경우(0)는 잘 분류하지만 살아있는 경우(1)는 분류하지 못하는 잘못된 모델이 만들어졌으며, 검증 데이터에서 0을 맞출 확률이 78%라고 나와도 이는 편향된 모델의 결과이므로 신뢰할 수 없다. 따라서 데이터 불균형을 해소하기 위해 업샘플링이나 다운샘플링이 필요했다.',
                    'result': '데이터 추가 전처리 필요 및 인위적 데이터 생성으로 인한 모델 신뢰도 부족'
                }
    ]
}